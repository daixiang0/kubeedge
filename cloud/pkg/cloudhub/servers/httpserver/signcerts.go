...
func GenerateToken() error {
	...
	if err := CreateTokenSecret([]byte(caHashToken)); err != nil {
		return fmt.Errorf("Failed to create the ca token  for edgecore register, err: %v", err)
	}
	...
	go func() {
		for {
			...
			if err := CreateTokenSecret([]byte(refreshedCaHashToken)); err != nil {
				klog.Fatalf("failed to craete the ca token for edgecore register, err: %v", err)
			}
		}
	}()

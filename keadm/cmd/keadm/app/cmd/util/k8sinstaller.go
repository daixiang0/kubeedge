...
func createKubeEdgeNs(client *kubernetes.Clientset) error {
	...
	_, err := client.CoreV1().Namespaces().Get(KubeEdgeCloudNameSpace, metav1.GetOptions{})
	if err != nil {
...
func installCloudFromFile(clientSet *kubernetes.Clientset) error {
	...
	for _, file := range files {
		...
		case files[1]:
			...
			if _, err = clientSet.CoreV1().ServiceAccounts(types.NameSpaceCloudCore).Create(serviceAccount); err != nil {
				return err
			}
		case files[2]:
		...
		case files[4]:
			...
			if _, err = clientSet.CoreV1().ConfigMaps(types.NameSpaceCloudCore).Create(configMap); err != nil {
				return err
			}
		case files[5]:
			...
			if _, err = clientSet.AppsV1().Deployments(types.NameSpaceCloudCore).Create(deployment); err != nil {
				return err
			}
		case files[6]:
			...
			if _, err = clientSet.CoreV1().Services(types.NameSpaceCloudCore).Create(service); err != nil {
				return err
			}
		}
	}
